...
#include "util/math.h"
#include <stdio.h>
void rtest(float x, float y, float a) {
    glm::vec2 r = rotate_around_origin({(x),(y)}, (a)); LOG_INF("%.1f,%.1f  rot %.0f deg = %.1f,%.1f", (x), (y), (a), r.x, r.y);
}
void rctest(float x, float y, float a, float cx, float cy) {
    glm::vec2 r = rotate_around_pt({(x),(y)}, {cx,cy}, (a)); LOG_INF("%.1f,%.1f  rot %.0f deg = %.1f,%.1f", (x), (y), (a), r.x, r.y);
}
...
void tmath() {
    ...
    rctest(0.f, 1.f, 90.f , 4.f, -2.f);
    rctest(0.f, 1.f, 45.f , 4.f, -2.f);
    rctest(0.f, 1.f, -45.f, 4.f, -2.f);
    // do {glm::vec2 r = rotate_around_origin({0.f,1.f}, 90.f); LOG_INF("%.1f,%.1f  rot %.0f deg = %.1f,%.1f", 0.f, 1.f, 90.f, r.x, r.y); } while (0);
}
